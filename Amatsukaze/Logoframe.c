...
int Logoframe(const char* avsfile, MLOGO_DATASET& logodata)
{
  const char* infile   = avsfile;
  int err_count = 0;
  int errforce = 0;
  int nopt_fromini = 0;
  int nopt_fromfile = 0;
  ...
  */
  ...
  const char *interlace_type = interlaced ? tff ? "t" : "b" : "p";
  const char *csp_type = NULL;
