...
int MultLogo_FileListGet(char *baselist[], const char *filename_src)
{
  ...
  struct stat st;
	...
	if (errnum == 0){
		...
		// add asterisk
		char* filename_src_duped  = strdup(filename_src);
		strext = strrchr(filename_src_duped, '.');      // get ext location
		free(filename_src_duped);
		if (strext != NULL){
...
int MultLogo_FileStrGet(char** name_dst, const char* name_src)
{
	...
	*name_dst = newstr;
	...
	char* name_src_duped = strdup(name_src);
	tmpstr = strchr(name_src_duped, '\"');
	free(name_src_duped);
